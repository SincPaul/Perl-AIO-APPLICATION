...
sub display_friend_list {
    ...
    my $num_friends = scalar(@$ordered_friends_ref);
    my $height = 30 * $num_friends + 50;
    $friend_list_overlay->geometry("350x$height");
    $friend_list_overlay->resizable(0, 0);
    $friend_list_overlay->configure(-background => 'lightgrey');

    my $online_frame = $friend_list_overlay->Frame(
        -bg => 'lightgreen'
    )->pack(-side => 'top', -fill => 'both', -expand => 1, -pady => 5, -padx => 5);

    my $offline_frame = $friend_list_overlay->Frame(
        -bg => 'lightyellow'
    )->pack(-side => 'top', -fill => 'both', -expand => 1, -pady => 5, -padx => 5);

    my $error_frame = $friend_list_overlay->Frame(
        -bg => 'lightcoral'
    )->pack(-side => 'top', -fill => 'both', -expand => 1, -pady => 5, -padx => 5);
    ...
    foreach my $friend (@$ordered_friends_ref) {
        ...
        if ($status eq 'online') {
            $button->pack(-in => $online_frame, -side => 'top', -fill => 'x', -padx => 10, -pady => 5);
        } elsif ($status eq 'offline') {
            $button->pack(-in => $offline_frame, -side => 'top', -fill => 'x', -padx => 10, -pady => 5);
        } else {
            $button->pack(-in => $error_frame, -side => 'top', -fill => 'x', -padx => 10, -pady => 5);
        }
    }
